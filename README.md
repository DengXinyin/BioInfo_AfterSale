...
- `plot_dot_heatmap()`、`plot_cross_heatmap()`：绘制 Spearman/Pearson 相关的点阵、矩形热图。
- `plot_circular_heatmap()`：绘制基因 × 样本的环形（环状）聚类热图——按行 Z-score 归一化、基因聚类树在圆环内侧、基因名标在外侧、样本名沿径向标注（基于 `circlize::circos.heatmap()`，起始角与扇区间隔可调）。
...
[使用 `GO_KEGG_plot()` 绘制 GO/KEGG 富集气泡图](docs/GO_KEGG.md)。

带分组色带、Gene count 气泡和基因标签的富集分析条形图示例，见
[使用 `enrichment_bar()` 绘制富集分析条形图](docs/enrichment_bar.md)。
...
完整示例见[使用 `Heatmap_plot()` 绘制带样本分组的表达热图](docs/Heatmap.md)。
...
可直接运行的模拟数据和输出示例见
[PCA、火山图、Venn 和 GSEA 可视化示例](docs/PCA.md)、[火山图](docs/Volcano.md)、[Venn](docs/Venn.md) 和 [GSEA](docs/GSEA.md)。
