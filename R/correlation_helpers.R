...
.correlation_stats <- function(x, y, method = "spearman") {
  rho <- .correlation_matrix(x, y, method); p <- matrix(NA_real_, nrow(rho), ncol(rho), dimnames = dimnames(rho))
  x <- as.data.frame(x, check.names = FALSE); y <- as.data.frame(y, check.names = FALSE); common <- intersect(rownames(x), rownames(y)); x <- x[common, , drop = FALSE]; y <- y[common, , drop = FALSE]; x[] <- lapply(x, as.numeric); y[] <- lapply(y, as.numeric)
  for (i in seq_len(ncol(x))) for (j in seq_len(ncol(y))) { ok <- is.finite(x[[i]]) & is.finite(y[[j]]); if (sum(ok) >= 3L && length(unique(x[[i]][ok])) > 1L && length(unique(y[[j]][ok])) > 1L) p[i, j] <- suppressWarnings(stats::cor.test(x[[i]][ok], y[[j]][ok], method = method, exact = FALSE)$p.value) }
  list(rho = rho, p = p)
}
.significance_stars <- function(p) ifelse(is.na(p), "", ifelse(p < .001, "***", ifelse(p < .01, "**", ifelse(p < .05, "*", ""))))
